...
export function LandingLeadForm({ content, locale }: Props) {
  ...
  const [processPrefill, setProcessPrefill] = useState("");
  ...
  useEffect(() => {
    ...
    try {
      ...
      const opts = Array.isArray(L.processOptions) ? L.processOptions : [];
      if (q && opts.some((o) => o.value === q)) {
        setProcessPrefill(q);
      }
    } catch {
      /* ignore */
  ...
  const form = (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <label className="block text-sm">
        ...
        <select
          key={processPrefill || "empty"}
          name="process"
          required
          defaultValue={processPrefill || ""}
