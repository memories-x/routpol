...
/**
 * Keep source page content; wrap each page in ROUTEPOL header/footer on A4.
...
export async function stampBrandedPdf(
  ...
  out.setTitle(`${head}`);
  out.setProducer("ROUTEPOL");
  out.setCreator("ROUTEPOL panel");
...
export function brandedFileName(originalName: string): string {
  ...
  if (/_ROUTEPOL$/i.test(base) || /ROUTEPOL/i.test(base)) {
    return `${base}.pdf`;
  }
  return `${base}_ROUTEPOL.pdf`;
}
