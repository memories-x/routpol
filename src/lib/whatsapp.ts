...
/** Visitor → ROUTEPOL WhatsApp number (site FAB / CTA). */
...
export function operatorWhatsAppPrefill(
  ...
  if (loc === "pl") {
    return `Dzień dobry ${name}, ROUTEPOL — piszę w sprawie zgłoszenia: ${svc}.`;
  }
  if (loc === "en") {
    return `Hello ${name}, this is ROUTEPOL regarding your request: ${svc}.`;
  }
  return `Merhaba ${name}, ROUTEPOL — ${svc} talebiniz hakkında yazıyorum.`;
}
