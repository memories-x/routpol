...
import { isLiveSubscriptionStatus } from "@/lib/customer-portal/stripe-subscription";

export const RENEWAL_REMINDER_DAYS = 5;

/** Dönem bitimine RENEWAL_REMINDER_DAYS gün kala hatırlatma (tek sefer).
 * Aktif Stripe aboneliği varsa atlanır — kart yenilemesi otomatik.
 */
export async function sendPeriodRenewalReminders(): Promise<{
  ...
  for (const period of periods) {
    if (
      period.customer.stripeSubscriptionId &&
      isLiveSubscriptionStatus(period.customer.stripeSubscriptionStatus)
    ) {
      skipped += 1;
      continue;
    }
...
export function isWithinRenewalReminderWindow(
  ...
  return (
    daysLeft >= RENEWAL_REMINDER_DAYS - 1 &&
    daysLeft <= RENEWAL_REMINDER_DAYS + 1
  );
}
