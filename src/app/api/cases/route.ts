...
const bodySchema = z
  .object({
    ...
    callRequestedAt: z.string().trim().max(40).optional(),
    city: z.string().trim().max(80).optional(),
  })
  .superRefine((data, ctx) => {
    if (data.packageSlug === "telefon-gorusme") {
      if (!data.callRequestedAt) {
        ctx.addIssue({
          code: "custom",
          path: ["callRequestedAt"],
          message: "Call date/time required",
        });
      }
      if (!data.notes || data.notes.trim().length < 10) {
        ctx.addIssue({
          code: "custom",
          path: ["notes"],
          message: "Topic briefing required",
        });
      }
    }
    if (data.packageSlug === "yerinde-eslik") {
...
export async function POST(request: Request) {
  ...
  if (isSubscriptionPackage(data.packageSlug)) {
    const { isDatabaseConfigured } = await import("@/lib/db");
    if (!isDatabaseConfigured()) {
      return NextResponse.json(
        fail(
          "UNAVAILABLE",
          "Subscription packages require DATABASE_URL (portal + renewals)",
        ),
        { status: 503 },
      );
    }
    const blocked = await recurringPackageBlock(email, data.packageSlug);
    if (blocked) {
  ...
  const metaRaw: Partial<CasePricingMeta> = {
    ...
    entityType: data.entityType,
    callRequestedAt: data.callRequestedAt,
    city: data.city,
  };
