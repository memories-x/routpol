...
import { formatPricingLine, packageLabelTr } from "@/lib/cases/package-labels";
...
export default async function PanelCasePage({ params }: PageProps) {
  ...
  const showUploads = packageRequiresDocumentUploads(record.packageSlug);
        ...
        <section className="rounded-lg border border-pt-border bg-pt-surface p-5">
          ...
          {letter ? (
            <p className="mt-4 rounded-md border border-pt-border bg-pt-bg px-3 py-2 text-xs text-pt-slate-600">
              Panel = canlı kayıt. Bilgisayarınızda aynı klasör adıyla
              mektup.pdf + sonuc.pdf tutun.
            </p>
          ) : null}
        </section>
          ...
          {showUploads ? (
            <>
              ...
              <p className="mt-1 text-sm text-pt-slate-600">
                Mektup (müşteri) ve sonuç (siz) — tıklayınca indirilir
              </p>
