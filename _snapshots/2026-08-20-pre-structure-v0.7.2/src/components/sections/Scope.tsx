...
export function Scope({ content }: ScopeProps) {
  ...
  return (
    <section
      id="sinirlar"
      className="scroll-mt-20 bg-pt-navy-950 py-16 text-white md:py-24"
    >
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2">
          <div className="rounded-xl border border-white/10 bg-white/5 p-6">
            ...
            <ul className="mt-4 space-y-3">
              {included.map((line) => (
                <li key={line} className="flex gap-3 text-sm text-pt-slate-200">
                  <Check
          ...
          <div className="rounded-xl border border-white/10 bg-pt-navy-900/60 p-6">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-pt-slate-400">
              {scope.excludedTitle}
            </h3>
            <ul className="mt-4 space-y-3">
              {excluded.map((line) => (
                <li key={line} className="flex gap-3 text-sm text-pt-slate-300">
                  <X
        ...
        <aside className="mt-8 flex gap-4 rounded-xl border border-pt-emerald-600/30 bg-pt-emerald-600/10 px-5 py-5 md:px-6">
          <Scale
