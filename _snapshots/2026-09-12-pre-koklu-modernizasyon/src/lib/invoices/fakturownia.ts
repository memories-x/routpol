import { isLetterPackage } from "@/lib/cases/document-slots";
import { productNameFor } from "@/lib/cases/pricing";
...
export async function createFakturowniaInvoice(
  ...
  const position: Record<string, unknown> = {
    name: (
      isLetterPackage(record.packageSlug)
        ? productNameFor(
            record.packageSlug,
            record.pricingMeta ?? {},
            record.locale,
          )
        : `POL-TURK — ${record.packageSlug}`
    ).slice(0, 256),
    quantity: 1,
    total_price_gross: Number(gross),
    tax: cfg.vat,
  };
