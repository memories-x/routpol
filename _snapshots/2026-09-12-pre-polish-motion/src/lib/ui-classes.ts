...
export function packageCardClassName(featured: boolean): string {
  return featured
    ? "relative flex h-full flex-col overflow-hidden rounded-2xl border border-pt-emerald-600/80 bg-pt-surface p-7 shadow-[0_20px_48px_-30px_rgba(5,150,105,0.4)]"
    : "flex h-full flex-col rounded-2xl border border-pt-border/90 bg-pt-surface p-6";
}
