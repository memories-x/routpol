...
- HttpOnly oturum; müşteri e-posta + şifre (`passwordHash`); magic link = şifre sıfırlama (hash + TTL + tek kullanım)
...
- Prod: veri sorumlusu unvan / NIP / adres yoksa `checkGoliveConfig` fail-close
- Rate limit (IP + e-posta)
- Müşteri eligibility kapısı (`PORTAL_ELIGIBLE_STATUSES`)
- `DELETE /api/customer/account` — profil anonimleştirme
- `purgeExpiredData()` — ödenmemiş 30 gün (meta + bytes), kapalı dosya 24 ay (`data_purged`)
...
| Veri kategorileri | Kimlik, iletişim, şifre hash (argon2), NIP (firma), evrak meta + dosya, ödeme meta (Stripe id) |
...
2. Kimlik doğrulama: e-posta + dosya no veya `/hesabim` oturumu
