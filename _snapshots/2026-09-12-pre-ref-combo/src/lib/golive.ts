...
/** Live checkout must not use Dashboard test keys. */
export function isStripeTestSecret(key: string | undefined): boolean {
  return (key ?? "").trim().startsWith("sk_test_");
}
...
export function checkGoliveConfig(): GoliveCheck {
  ...
  if (!process.env.STRIPE_SECRET_KEY) {
    ...
    isStripeTestSecret(process.env.STRIPE_SECRET_KEY) &&
    isProductionRuntime()
  ) {
    missing.push(
      "STRIPE_SECRET_KEY (live sk_live_ required in production, not sk_test_)",
    );
  }
  ...
  const identity = [
    ["NEXT_PUBLIC_CONTACT_EMAIL", "NEXT_PUBLIC_CONTACT_EMAIL (site contact)"],
    [
      "NEXT_PUBLIC_OPERATOR_LEGAL_NAME",
      "NEXT_PUBLIC_OPERATOR_LEGAL_NAME (privacy page)",
    ],
    [
      "NEXT_PUBLIC_OPERATOR_NIP",
      "NEXT_PUBLIC_OPERATOR_NIP (privacy / invoice identity)",
    ],
    [
      "NEXT_PUBLIC_OPERATOR_ADDRESS",
      "NEXT_PUBLIC_OPERATOR_ADDRESS (privacy page)",
    ],
    ["CRON_SECRET", "CRON_SECRET (scheduled cleanup/outbox)"],
  ] as const;

  for (const [key, label] of identity) {
    if (process.env[key]?.trim()) continue;
    if (isProductionRuntime()) missing.push(key);
