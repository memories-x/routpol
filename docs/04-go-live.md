...
6. Stripe **live** (`sk_live_…`) + webhook `https://DOMAIN/api/webhooks/stripe` + `STRIPE_WEBHOOK_SECRET` (Dashboard invoice/receipt e-postaları **kapalı**)
7. Yerel fatura: `INVOICE_PROVIDER=local` + `SELLER_*` + `SMTP_*` (`docs/51-stripe-ksef-faturalar.md`); `INVOICE_AUTO_ISSUE` boş
8. Resend + SPF/DKIM; `RESEND_API_KEY`, `CONTACT_EMAIL_FROM` / `_TO` (lead/portal; fatura SMTP ayrı)
9. Panel: `PANEL_PASSWORD`, `PANEL_AUTH_SECRET`, `CUSTOMER_AUTH_SECRET`, `CRON_SECRET`
10. Plesk günlük cron: `scripts/plesk-cron.example.sh`
11. `npm run golive:check -- --strict` yeşil
12. Üç gerçek smoke (`docs/04-go-live.md` smoke)
13. RODO imza (`docs/05-uyum-rodo.md`) — kod şablon; imza sizin
...
**Panelsiz Ubuntu (önerilen VPS yolu):** `docs/55-ubuntu-caddy-multi-site.md` — Caddy + systemd + site başına PORT/DB.
